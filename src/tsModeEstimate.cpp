...
std::vector<double> tsModeEstimate(const std::vector<double> &xVec) {
  ...
  if (numModes > 1) {
    std::cout << "Taut string disagrees with DIP; returning sample median." << std::endl;
    ...
    if (cutCounter == 0) {
      std::cout << "Warning: tsModeEstimate string found no mode. Returning sample median." << std::endl;
      modeGates.push_back(*std::min_element(xVec.begin(),xVec.end()));
      modeGates.push_back(sampleMedian);
      modeGates.push_back(*std::max_element(xVec.begin(),xVec.end()));
      std::sort(modeGates.begin(),modeGates.end()); //ensure modeGates are returned in ascending order
      ...
      if (cutCounter > 1) {
	std::cout << "Warning: tsModeEstimate finding multiple modes. Using first as cut-point." << std::endl;
      }
      yInds.clear();
      cCut = cutValue[0];
